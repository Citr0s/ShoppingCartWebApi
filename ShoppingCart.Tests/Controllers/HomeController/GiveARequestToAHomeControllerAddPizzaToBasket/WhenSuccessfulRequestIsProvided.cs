...
namespace ShoppingCart.Tests.Controllers.HomeController.GiveARequestToAHomeControllerAddPizzaToBasket
{
    ...
    public class WhenSuccessfulRequestIsProvided
    {
        ...
        {
            _userSessionService.Verify(
                x => x.AddItemToBasket(It.IsAny<string>(), It.Is<BasketData>(y => y.ExtraToppingIds.Contains(value))),
                Times.Once);
        }
        ...
        {
            _userSessionService.Verify(
                x => x.AddItemToBasket(It.IsAny<string>(), It.Is<BasketData>(y => y.PizzaId == 1)), Times.Once);
        }
        ...
        {
            _userSessionService.Verify(
                x => x.AddItemToBasket(It.IsAny<string>(), It.Is<BasketData>(y => y.SizeId == 1)), Times.Once);
        }
        ...
        {
            _userSessionService.Verify(
                x => x.AddItemToBasket(It.Is<string>(y => y == "SomeUserIdentifier"), It.IsAny<BasketData>()),
                Times.Once);
        }
    }
}